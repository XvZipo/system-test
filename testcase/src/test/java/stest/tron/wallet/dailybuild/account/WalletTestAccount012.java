...
public class WalletTestAccount012 {
  ...
  public void beforeClass() throws Exception {
    ...
    if (!PublicMethed.tronPowerProposalIsOpen(blockingStubFull)
        || !PublicMethed.freezeV2ProposalIsOpen(blockingStubFull)) {
      if (channelFull != null) {
  ...
  public void test01FreezeBalanceGetTronPower() {
    ...
    Assert.assertTrue(
        PublicMethed.sendcoin(
            frozenAddress, sendAmount, foundationAddress, foundationKey, blockingStubFull));
    PublicMethed.waitProduceNextBlock(blockingStubFull);

    AccountResourceMessage accountResource =
        PublicMethed.getAccountResource(frozenAddress, blockingStubFull);
    ...
    Assert.assertTrue(
        PublicMethed.freezeBalanceGetTronPower(
            frozenAddress, frozenAmountForTronPower, 0, 2, null, frozenKey, blockingStubFull));
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    ...
    Assert.assertEquals(
        PublicMethed.freezeV2ProposalIsOpen(blockingStubFull)
            ? account.getFrozenV2(2).getAmount()
            : account.getTronPower().getFrozenBalance(),
        frozenAmountForTronPower);
    Assert.assertTrue(
        PublicMethed.freezeV2ProposalIsOpen(blockingStubFull)
            ? true
            : account.getTronPower().getExpireTime() > beforeFrozenTime
                && account.getTronPower().getExpireTime() < afterFrozenTime);
    accountResource = PublicMethed.getAccountResource(frozenAddress, blockingStubFull);
    ...
    Assert.assertEquals(
        afterTotalTronPowerWeight - beforeTotalTronPowerWeight,
        frozenAmountForTronPower / 1000000L);

    Assert.assertEquals(
        afterTronPowerLimit - beforeTronPowerLimit, frozenAmountForTronPower / 1000000L);
    ...
    Assert.assertEquals(afterTronPowerLimit - beforeTronPowerLimit, 6);
  }
  ...
  public void test02VotePowerOnlyComeFromTronPower() {
    ...
    Assert.assertFalse(
        PublicMethed.voteWitness(frozenAddress, frozenKey, witnessMap, blockingStubFull));
    ...
    Long afterTronPowerUsed = accountResource.getTronPowerUsed();
    Assert.assertEquals(
        afterTronPowerUsed - beforeTronPowerUsed, frozenAmountForTronPower / 1000000L);
    ...
    Assert.assertEquals(secondBeforeTronPowerUsed - afterTronPowerUsed, 1);
  }
  ...
  public void test04UnfreezeBalanceForTronPower() {
    AccountResourceMessage accountResource =
        PublicMethed.getAccountResource(foundationAddress, blockingStubFull);
    final Long beforeTotalTronPowerWeight = accountResource.getTotalTronPowerWeight();
    Long canUnfreezeAmount = PublicMethed.getFrozenV2Amount(frozenAddress, 2, blockingStubFull);
    Assert.assertTrue(
        PublicMethed.unFreezeBalanceV2(
            frozenAddress, frozenKey, canUnfreezeAmount, 2, blockingStubFull));
    PublicMethed.waitProduceNextBlock(blockingStubFull);

    accountResource = PublicMethed.getAccountResource(frozenAddress, blockingStubFull);
    Long afterTotalTronPowerWeight = accountResource.getTotalTronPowerWeight();
    Assert.assertTrue(
        beforeTotalTronPowerWeight - afterTotalTronPowerWeight == canUnfreezeAmount / 1000000);
  ...
  public void shutdown() throws InterruptedException {
    PublicMethed.unFreezeBalance(frozenAddress, frozenKey, 2, null, blockingStubFull);
