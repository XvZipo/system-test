...
public class WalletTestAssetIssue020 {
  ...
  public void test01AssetIssueSupportPrecision() {
    ...
    Assert.assertTrue(
        PublicMethed.sendcoin(
            asset020Address, 204800000000L, fromAddress, testKey002, blockingStubFull));
    Assert.assertTrue(
        PublicMethed.sendcoin(
            asset020SecondAddress, 204800000000L, fromAddress, testKey002, blockingStubFull));
    ...
    AssetIssueContractOuterClass.AssetIssueContract assetIssueInfo =
        PublicMethed.getAssetIssueByName(name, blockingStubFull);
    ...
    assetIssueInfo =
        PublicMethed.getAssetIssueById(
            ByteArray.toStr(assetAccountId.toByteArray()), blockingStubFull);
    final Integer preCisionById = assetIssueInfo.getPrecision();
    final Long TotalSupplyById = assetIssueInfo.getTotalSupply();

    assetIssueInfo =
        PublicMethed.getAssetIssueListByName(name, blockingStubFull).get().getAssetIssue(0);
  ...
  public void test02GetAssetIssueByidFromSolidity() {
    Assert.assertEquals(
        PublicMethed.getAssetIssueById(
            ByteArray.toStr(assetAccountId.toByteArray()), blockingStubFull),
        PublicMethed.getAssetIssueByIdFromSolidity(
            ByteArray.toStr(assetAccountId.toByteArray()), blockingStubSolidity));
    Assert.assertEquals(
        PublicMethed.getAssetIssueById(
            ByteArray.toStr(assetAccountId.toByteArray()), blockingStubFull),
        PublicMethed.getAssetIssueByIdFromSolidity(
            ByteArray.toStr(assetAccountId.toByteArray()), blockingStubSoliInFull));
  }
  ...
  public void test03GetAssetIssueByIdFromPbft() {
    Assert.assertEquals(
        PublicMethed.getAssetIssueById(
            ByteArray.toStr(assetAccountId.toByteArray()), blockingStubFull),
        PublicMethed.getAssetIssueByIdFromSolidity(
            ByteArray.toStr(assetAccountId.toByteArray()), blockingStubPbft));
  }
  ...
  public void test04GetAssetIssueListByNameFromSolidity() {
    Assert.assertEquals(
        PublicMethed.getAssetIssueListByNameFromSolidity(name, blockingStubSolidity)
            .get()
            .getAssetIssueList()
            .get(0)
            .getTotalSupply(),
  ...
  public void test05GetAssetIssueListByNameFromPbft() {
    Assert.assertEquals(
        PublicMethed.getAssetIssueListByNameFromSolidity(name, blockingStubPbft)
            .get()
            .getAssetIssue(0)
            .getTotalSupply(),
        totalSupply);
  ...
  @Test(
      enabled = true,
      description = "freeAssetNetUsed decreasing " + "in getAssetIssueById and getAccount")
  public void test06NetUsedDecrease() {
    ...
    Assert.assertTrue(
        PublicMethed.sendcoin(
            assetOwnerAddress, 200000000000L, fromAddress, testKey002, blockingStubFull));
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    Assert.assertTrue(
        PublicMethed.freezeBalanceForReceiver(
            fromAddress,
            PublicMethed.getFreezeBalanceCount(
                assetOwnerAddress, assetOwnerKey, 500000L, blockingStubFull),
            0,
            1,
            ByteString.copyFrom(assetOwnerAddress),
            testKey002,
            blockingStubFull));
    Assert.assertTrue(
        PublicMethed.freezeBalanceForReceiver(
    ...
    Account getAssetIdFromThisAccount =
        PublicMethed.queryAccount(assetOwnerAddress, blockingStubFull);
    assetAccountId = getAssetIdFromThisAccount.getAssetIssuedID();
    ...
    Assert.assertTrue(
        PublicMethed.sendcoin(
            assetFromAddress, 100000000000L, fromAddress, testKey002, blockingStubFull));
    ...
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    Long fromBeginNetUsed =
        PublicMethed.queryAccount(assetFromAddress, blockingStubFull)
            .getFreeAssetNetUsageV2Map()
            .get(strAssetId)
            .longValue();
    Long assetBeginNetUsed =
        PublicMethed.getAssetIssueById(strAssetId, blockingStubFull).getPublicFreeAssetNetUsage();
