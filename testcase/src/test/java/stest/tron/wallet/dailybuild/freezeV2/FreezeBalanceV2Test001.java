...
public class FreezeBalanceV2Test001 {
  ...
  public void test01FreezeBalanceV2GetBandwidth() throws Exception {
    ...
    FreezeBalanceV2Contract freezeBalanceV2Contract = any.unpack(FreezeBalanceV2Contract.class);
    Assert.assertTrue(freezeBalanceV2Contract.getFrozenBalance() == freezeBandwidthBalance);
    Assert.assertEquals(
        freezeBalanceV2Contract.getOwnerAddress().toByteArray(), frozenBandwidthAddress);
  ...
  public void test02FreezeBalanceV2GetEnergy() throws Exception {
    ...
    accountResource = PublicMethed.getAccountResource(frozenEnergyAddress, blockingStubFull);
  ...
  public void test03UnFreezeBalanceV2ToReleaseBandwidth() throws Exception {
    ...
    Assert.assertEquals(
        unfreezeBalanceV2Contract.getOwnerAddress().toByteArray(), frozenBandwidthAddress);
    ...
    Assert.assertTrue(
        account.getUnfrozenV2(0).getUnfreezeExpireTime() > afterUnfreezeTimestamp
            && account.getUnfrozenV2(0).getUnfreezeExpireTime()
                <= afterUnfreezeTimestamp + periodTime);
  }
  ...
  public void test04UnFreezeBalanceV2ToReleaseEnergy() throws Exception {
    ...
    String txId =
        PublicMethed.unFreezeBalanceV2AndGetTxId(
            frozenEnergyAddress, frozenEnergyKey, freezeEnergyBalance, 1, blockingStubFull);
    Assert.assertNotNull(txId);
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    ...
    Assert.assertEquals(
        unfreezeBalanceV2Contract.getOwnerAddress().toByteArray(), frozenEnergyAddress);
  ...
  public void test05WithdrawExpireUnfreezeToReleaseBalance() throws Exception {
    ...
    if (System.currentTimeMillis() - 2000L < account.getUnfrozenV2(0).getUnfreezeExpireTime()) {
      Assert.assertFalse(
          PublicMethed.withdrawExpireUnfreeze(
              frozenEnergyAddress, frozenEnergyKey, blockingStubFull));
      ...
      while (retryTimes++ <= periodTime / 6000L
          && System.currentTimeMillis() < unfreezeExpireTime) {
        PublicMethed.waitProduceNextBlock(blockingStubFull);
      }
    }

    Long canWithdrawUnFreezeAmount =
        PublicMethed.getCanWithdrawUnfreezeAmount(
                frozenEnergyAddress, System.currentTimeMillis(), blockingStubFull)
            .get()
            .getAmount();
    // query solidity
    PublicMethed.waitSolidityNodeSynFullNodeData(blockingStubFull, blockingStubFullSolidity);
    Long canWithdrawUnFreezeAmountSolidity =
        PublicMethed.getCanWithdrawUnfreezeAmountSolidity(
                frozenEnergyAddress, System.currentTimeMillis(), blockingStubFullSolidity)
            .get()
            .getAmount();
    ...
    Long canWithdrawUnFreezeAmountPbft =
        PublicMethed.getCanWithdrawUnfreezeAmountSolidity(
                frozenEnergyAddress, System.currentTimeMillis(), blockingStubPbft)
            .get()
            .getAmount();
    Assert.assertEquals(canWithdrawUnFreezeAmount, canWithdrawUnFreezeAmountPbft);

    String txIdBandwidth =
        PublicMethed.withdrawExpireUnfreezeAndGetTxId(
    ...
    Transaction transaction = PublicMethed.getTransactionById(txIdEnergy, blockingStubFull).get();
    Any any = transaction.getRawData().getContract(0).getParameter();
    WithdrawExpireUnfreezeContract withdrawExpireUnfreezeContract =
        any.unpack(WithdrawExpireUnfreezeContract.class);
    Assert.assertEquals(
        withdrawExpireUnfreezeContract.getOwnerAddress().toByteArray(), frozenEnergyAddress);
    ...
    TransactionInfo transactionInfo =
        PublicMethed.getTransactionInfoById(txIdEnergy, blockingStubFull).get();
