...
public class FreezeBalanceV2Test002 {
  ...
  public void beforeClass() throws Exception {
    ...
    Assert.assertTrue(
        PublicMethed.sendcoin(
            frozenEnergyAddress, sendAmount, foundationAddress, foundationKey, blockingStubFull));
    Assert.assertTrue(
        PublicMethed.sendcoin(
            receiveBandwidthAddress, 1L, foundationAddress, foundationKey, blockingStubFull));
    Assert.assertTrue(
        PublicMethed.sendcoin(
            receiveEnergyAddress, 1L, foundationAddress, foundationKey, blockingStubFull));
    Assert.assertTrue(
        PublicMethed.sendcoin(
            delegateFromAddress, sendAmount, foundationAddress, foundationKey, blockingStubFull));
    ...
    Assert.assertTrue(
        PublicMethed.freezeBalanceV2(
            frozenEnergyAddress, freezeEnergyBalance, 1, frozenEnergyKey, blockingStubFull));
    PublicMethed.waitProduceNextBlock(blockingStubFull);
  }
  ...
  public void test01DelegateResourceOfBandwidth() throws Exception {
    ...
    Assert.assertEquals(
        delegateResourceContract.getOwnerAddress().toByteArray(), frozenBandwidthAddress);
    Assert.assertTrue(delegateResourceContract.getResourceValue() == 0);
    Assert.assertEquals(
        delegateResourceContract.getReceiverAddress().toByteArray(), receiveBandwidthAddress);

    account = PublicMethed.queryAccount(frozenBandwidthAddress, blockingStubFull);
    accountResource = PublicMethed.getAccountResource(frozenBandwidthAddress, blockingStubFull);
    ...
    final Long afterLenderAmount =
        account.getAcquiredDelegatedFrozenBalanceForBandwidth()
            + account.getAcquiredDelegatedFrozenV2BalanceForBandwidth()
            + afterLenderFrozenAmount;
    ...
    accountResource = PublicMethed.getAccountResource(receiveBandwidthAddress, blockingStubFull);
    ...
    final Long receiverAcquiredDelegatedFrozenBalanceForBandwidth =
        account.getAcquiredDelegatedFrozenV2BalanceForBandwidth();
    final Long afterReceiverAmount =
        account.getAcquiredDelegatedFrozenBalanceForBandwidth()
            + account.getAcquiredDelegatedFrozenV2BalanceForBandwidth()
  ...
  public void test02UnDelegateResourceOfBandwidth() throws Exception {
    ...
    account = PublicMethed.queryAccount(receiveBandwidthAddress, blockingStubFull);
    final Long beforeAcquiredDelegatedFrozenBalanceForBandwidth =
        account.getAcquiredDelegatedFrozenBalanceForBandwidth();
    ...
    Assert.assertEquals(
        unDelegateResourceContract.getOwnerAddress().toByteArray(), frozenBandwidthAddress);
    Assert.assertTrue(unDelegateResourceContract.getResourceValue() == 0);
    Assert.assertEquals(
        unDelegateResourceContract.getReceiverAddress().toByteArray(), receiveBandwidthAddress);
  ...
  public void test03TestGetCanDelegatedMaxSizeApi() throws Exception {
    Long canDelegatedMaxSizeWithNoNetUsed =
        PublicMethed.getCanDelegatedMaxSize(frozenBandwidthAddress, 0, blockingStubFull)
            .get()
            .getMaxSize();
    ...
    Long canDelegatedMaxSizeWithNoNetUsedSolidity =
    ...
    Long canDelegatedMaxSizeWithNoNetUsedPbft =
        PublicMethed.getCanDelegatedMaxSizeSolidity(frozenBandwidthAddress, 0, blockingStubPbft)
            .get()
            .getMaxSize();
    logger.info("canDelegatedMaxSizeWithNoNetUsedPbft" + canDelegatedMaxSizeWithNoNetUsedPbft);
    int retryTimes = 0;
    while (PublicMethed.getCanDelegatedMaxSize(frozenBandwidthAddress, 0, blockingStubFull)
                    .get()
                    .getMaxSize()
                + 1100000
            > canDelegatedMaxSizeWithNoNetUsed
        && retryTimes++ <= 5000) {
      logger.info(
          "Current"
              + PublicMethed.getCanDelegatedMaxSize(frozenBandwidthAddress, 0, blockingStubFull)
      ...
      PublicMethed.sendcoinGetTransactionIdForConstructData(
          foundationAddress, 1L, frozenBandwidthAddress, frozenBandwidthKey, blockingStubFull);
      Thread.sleep(10L);
    }

    PublicMethed.waitProduceNextBlock(blockingStubFull);
    Long canDelegatedMaxSizeWithNetUsed =
        PublicMethed.getCanDelegatedMaxSize(frozenBandwidthAddress, 0, blockingStubFull)
            .get()
            .getMaxSize();
    ...
    // query solidity
    PublicMethed.waitSolidityNodeSynFullNodeData(blockingStubFull, blockingStubFullSolidity);
    Long canDelegatedMaxSizeWithNetUsedSolidity =
        PublicMethed.getCanDelegatedMaxSizeSolidity(
                frozenBandwidthAddress, 0, blockingStubFullSolidity)
            .get()
            .getMaxSize();
    logger.info(
        "canDelegatedMaxSizeWithNetUsedSolidity: " + canDelegatedMaxSizeWithNetUsedSolidity);
    // query pbft
    PublicMethed.waitSolidityNodeSynFullNodeData(blockingStubFull, blockingStubPbft);
    Long canDelegatedMaxSizeWithNetUsedPbft =
        PublicMethed.getCanDelegatedMaxSizeSolidity(frozenBandwidthAddress, 0, blockingStubPbft)
            .get()
            .getMaxSize();
    logger.info("canDelegatedMaxSizeWithNetUsedPbft: " + canDelegatedMaxSizeWithNetUsedPbft);
    Assert.assertEquals(
        canDelegatedMaxSizeWithNetUsedPbft.longValue(),
        canDelegatedMaxSizeWithNetUsedSolidity.longValue());

    Assert.assertTrue(
        PublicMethed.freezeBalanceV2(
            delegateFromAddress, freezeBandwidthBalance, 0, delegateFromKey, blockingStubFull));
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    Assert.assertTrue(
        PublicMethed.delegateResourceV2(
            delegateFromAddress,
    ...
    Long canDelegatedMaxSizeWithOtherDelegatedToMe =
        PublicMethed.getCanDelegatedMaxSize(frozenBandwidthAddress, 0, blockingStubFull)
            .get()
            .getMaxSize();
  ...
  public void shutdown() throws InterruptedException {
    PublicMethed.freedResource(
        frozenBandwidthAddress, frozenBandwidthKey, foundationAddress, blockingStubFull);
    if (channelFull != null) {
      channelFull.shutdown().awaitTermination(5, TimeUnit.SECONDS);
    }
  }
