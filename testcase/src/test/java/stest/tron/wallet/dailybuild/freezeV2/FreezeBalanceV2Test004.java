...
public class FreezeBalanceV2Test004 {
  ...
  public void test01MaxUnfreezeBalanceListIs32() throws Exception {
    ...
    // query solidity
    Assert.assertTrue(
        PublicMethed.getAvailableUnfreezeCountSolidity(
                    frozenBandwidthAddress, blockingStubFullSolidity)
                .get()
                .getCount()
            == 32);
    int unfreezeTimes = 0;
    while (unfreezeTimes++ <= 50) {
      PublicMethed.unFreezeBalanceV2(
          frozenBandwidthAddress, frozenBandwidthKey, unfreezeBalance, 0, blockingStubFull);
      Thread.sleep(100L);
    }
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    ...
    Long count =
        PublicMethed.getAvailableUnfreezeCount(frozenBandwidthAddress, blockingStubFull)
            .get()
            .getCount();
    Account accountFrozenBandwidthAddress =
        PublicMethed.queryAccount(frozenBandwidthAddress, blockingStubFull);
    logger.info("account++: ");
    ...
    // query pbft
    Assert.assertTrue(
        PublicMethed.getAvailableUnfreezeCountSolidity(frozenBandwidthAddress, blockingStubPbft)
                .get()
                .getCount()
            == 0);
    Assert.assertTrue(
    ...
    int retryTimes = 0;
    while (retryTimes++ <= periodTime / 3000L
        && System.currentTimeMillis() - 4000 <= maxUnfreezeExpireTime) {
  ...
  public void test02UnfreezeCaseDynamicDecreaseOfVote() throws Exception {
    ...
    Account account = PublicMethed.queryAccount(frozenBandwidthAddress, blockingStubFull);
    Long votePower =
        !PublicMethed.tronPowerProposalIsOpen(blockingStubFull)
    ...
    Assert.assertTrue(
        PublicMethed.voteWitness(
            frozenBandwidthAddress, frozenBandwidthKey, voteMap, blockingStubFull));
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    ...
    Assert.assertTrue(
        PublicMethed.unFreezeBalanceV2(
            ...
            frozenBandwidthKey,
            unfreezeBalance,
            PublicMethed.tronPowerProposalIsOpen(blockingStubFull) ? 2 : 0,
            blockingStubFull));
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    ...
    Assert.assertTrue(
        PublicMethed.unFreezeBalanceV2(
            frozenBandwidthAddress,
            frozenBandwidthKey,
            (currentVote - witness1ReceiveVote / 10 - witness2ReceiveVote / 10) * 1000000L,
            PublicMethed.tronPowerProposalIsOpen(blockingStubFull) ? 2 : 0,
            blockingStubFull));
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    account = PublicMethed.queryAccount(frozenBandwidthAddress, blockingStubFull);
    ...
    if (PublicMethed.tronPowerProposalIsOpen(blockingStubFull)) {
      Assert.assertTrue(
          PublicMethed.freezeBalanceV2(
              frozenBandwidthAddress,
              (witness1ReceiveVote + witness2ReceiveVote) * 1000000L,
              2,
              frozenBandwidthKey,
              blockingStubFull));
    } else {
