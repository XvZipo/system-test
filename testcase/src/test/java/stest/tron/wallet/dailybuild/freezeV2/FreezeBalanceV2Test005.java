...
public class FreezeBalanceV2Test005 {
  ...
  private final String witness1Key =
  ...
  private final String witness2Key =
      Configuration.getByPath("testng.conf").getString("witness.key1");
  ...
  public void beforeClass() throws Exception {
    ...
    contractAddress =
        PublicMethed.getTransactionInfoById(txid, blockingStubFull)
            ...
            .toByteArray();
    Assert.assertEquals(
        PublicMethed.queryAccount(contractAddress, blockingStubFull).getType(),
  ...
  public void test01BandwidthNetUsageChangeNetWindowSIze() throws Exception {
    ...
    Long transactionNetUsage =
        PublicMethed.getTransactionInfoById(txid, blockingStubFull)
            .get()
            .getReceipt()
            .getNetUsage();
    Assert.assertTrue(
        afterNetUsage <= transactionNetUsage
    ...
    Assert.assertTrue(
        afterNetWindowSize > beforeNetWindowSize && afterNetWindowSize <= 28795 * 1000);
    ...
    transactionNetUsage =
        PublicMethed.getTransactionInfoById(txid, blockingStubFull)
            ...
            .getNetUsage();
    Assert.assertTrue(
        afterNetUsage - beforeNetUsage <= transactionNetUsage
  ...
  public void test02EnergyUsageChangeEnergyWindowSizeWithTriggerSuccess() throws Exception {
    ...
    Long transactionEnergyUsage =
        PublicMethed.getTransactionInfoById(txid, blockingStubFull)
            .get()
            .getReceipt()
            .getEnergyUsage();
    Assert.assertEquals(
        PublicMethed.getTransactionInfoById(txid, blockingStubFull).get().getReceipt().getResult(),
        contractResult.SUCCESS);
    logger.info("transactionEnergyUsage:" + transactionEnergyUsage);
    Assert.assertTrue(
        afterEnergyUsage <= transactionEnergyUsage
    ...
    Assert.assertTrue(
        afterEnergyWindowSize <= 28795 * 1000 && afterEnergyWindowSize > beforeEnergyWindowSize);
    afterEnergyUsage = account.getAccountResource().getEnergyUsage();
    transactionEnergyUsage =
        PublicMethed.getTransactionInfoById(txid, blockingStubFull)
            .get()
            .getReceipt()
            .getEnergyUsage();
    Assert.assertTrue(
        afterEnergyUsage - beforeEnergyUsage <= transactionEnergyUsage
            && afterEnergyUsage - beforeEnergyUsage + 8 >= transactionEnergyUsage
            && transactionEnergyUsage > 550
            && afterEnergyUsage > 550 * 2);
  }
