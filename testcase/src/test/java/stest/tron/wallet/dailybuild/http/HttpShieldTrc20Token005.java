...
@Slf4j
public class HttpShieldTrc20Token005 extends AbstractZenTrc20HttpNodes012 {
  ...
  public void test01ShiledTrc20BurnToOnePublicAndOneShieldByHttp() {
    ...
    shieldedReceives =
        getHttpShieldedReceivesJsonArray(
            shieldedReceives,
            toShieldAmount,
            shieldReceiverAccountInfo.getString("payment_address"),
            getRcm(httpnode));
    ...
    Assert.assertEquals(
        noteTxs.getJSONObject(0).getJSONObject("note").getLong("value"), toShieldAmount);
    Assert.assertEquals(
        noteTxs.getJSONObject(0).getJSONObject("note").getString("payment_address"),
        shieldReceiverAccountInfo.getString("payment_address"));

    Assert.assertEquals(noteTxs.getJSONObject(1).getLong("to_amount"), toPublicAmount);
    Assert.assertEquals(
        noteTxs.getJSONObject(1).getString("transparent_to_address"),
        zenTrc20TokenOwnerAddressString);
  ...
  @Test(
      enabled = true,
      description = "Shield trc20 burn without ask to one " + "public and one shield by http")
  public void test02ShiledTrc20BurnWithoutAskToOnePublicAndOneShieldByHttp() {
    ...
    Assert.assertEquals(
        noteTxs.getJSONObject(2).getJSONObject("note").getLong("value"), toShieldAmount);
    Assert.assertEquals(
        noteTxs.getJSONObject(2).getJSONObject("note").getString("payment_address"),
        shieldReceiverAccountInfo.getString("payment_address"));

    Assert.assertEquals(noteTxs.getJSONObject(3).getLong("to_amount"), toPublicAmount);
    Assert.assertEquals(
        noteTxs.getJSONObject(3).getString("transparent_to_address"),
        zenTrc20TokenOwnerAddressString);
