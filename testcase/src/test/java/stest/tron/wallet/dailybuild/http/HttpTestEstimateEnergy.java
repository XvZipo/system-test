...
import stest.tron.wallet.common.client.AbstractHttpEstimateEnergyNodes156;
...
@Slf4j
public class HttpTestEstimateEnergy extends AbstractHttpEstimateEnergyNodes156 {
  ...
  public void beforeClass() {
    ...
    Assert.assertTrue(
        PublicMethed.sendcoin(
            triggerAddress, 10000000000L, fromAddress, testKey002, blockingStubFull));
  ...
  public void test01EstimateCanGetValue() {
    ...
    response =
        HttpMethed.getEstimateEnergyPBFT(
  ...
  public void test02CompareToTriggerConstantContract() {
    ...
    response =
        HttpMethed.getEstimateEnergy(
            httpnode, fromAddress, contractAddress, method, param, null, true, 0, 0, 0);
    Long energyRequired = HttpMethed.parseResponseContent(response).getLong("energy_required");
    Assert.assertTrue(energyRequired >= 0);
    response =
        HttpMethed.triggerConstantContract(
            httpnode, fromAddress, ByteArray.toHexString(contractAddress), method, param);
    Long energyRequiredConstant = HttpMethed.parseResponseContent(response).getLong("energy_used");
  ...
  public void test03EstimateDeployContract() {
    ...
    response =
        HttpMethed.getEstimateEnergy(
            httpnode, fromAddress, null, method, param, code, true, 0, 0, 0);
    Long energyRequired = HttpMethed.parseResponseContent(response).getLong("energy_required");
    Assert.assertTrue(energyRequired >= 0);
    response =
  ...
  @Test(
      enabled = true,
      description = "Estimate function out_of_time by estimateEnergy and triggerConstant")
  public void test04EstimateWithCallvalueAndCommandAfterCall() {
    ...
    response =
        HttpMethed.getEstimateEnergy(
            httpnode, fromAddress, contractAddress, method, args, null, true, 0, 0, 0);
    responseContent = HttpMethed.parseResponseContent(response);
    ...
    Assert.assertEquals("OTHER_ERROR".toLowerCase(), res.getString("code").toLowerCase());
    Assert.assertTrue(
        ByteString.copyFrom(ByteArray.fromHexString(res.getString("message")))
            .toStringUtf8()
            .contains("CPU timeout"));
  }
  ...
  @Test(
      enabled = true,
      description =
          "estimateEnergy and triggerconstantcontract "
  ...
  public void test05EstimateOnlyHasCalldata() {
    ...
    response =
        HttpMethed.getEstimateEnergy(
            httpnode, fromAddress, contractAddress, null, null, data, true, 0, 0, 0);
    Long energyRequired = HttpMethed.parseResponseContent(response).getLong("energy_required");
    Assert.assertTrue(energyRequired >= 0);
    response =
        HttpMethed.triggerConstantContractWithData(
  ...
  public void test06EstimateOnlyContractAddress() {
    response =
        HttpMethed.getEstimateEnergy(
            httpnode, fromAddress, contractAddress, null, null, null, true, 0, 0, 0);
    Long energyRequired = HttpMethed.parseResponseContent(response).getLong("energy_required");
    Assert.assertTrue(energyRequired >= 0);
    response =
        HttpMethed.triggerConstantContractWithData(
  ...
  @Test(
      enabled = true,
      description =
          "estimateEnergy and triggerconstantcontract with contract address, function_selector and"
              + " data. and it will triggerContract use function_selector ")
  public void test07EstimatePreferFunctionSelector() {
    ...
    response =
        HttpMethed.getEstimateEnergy(
            httpnode, fromAddress, contractAddress, method, param, data, true, 0, 0, 0);
    Long energyRequired = HttpMethed.parseResponseContent(response).getLong("energy_required");
    Assert.assertTrue(energyRequired >= 0);
    response =
        HttpMethed.triggerConstantContractWithData(
