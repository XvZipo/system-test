...
public class ContractInternalTransaction003 {
  ...
  public void testInternalTransaction013() {
    ...
    String note =
        ByteArray.toStr(infoById.get().getInternalTransactions(0).getNote().toByteArray());
    String note1 =
        ByteArray.toStr(infoById.get().getInternalTransactions(1).getNote().toByteArray());
    String note2 =
        ByteArray.toStr(infoById.get().getInternalTransactions(2).getNote().toByteArray());
    String note3 =
        ByteArray.toStr(infoById.get().getInternalTransactions(3).getNote().toByteArray());
  ...
  public void testInternalTransaction014() {
    ...
    Assert.assertEquals(
        ByteArray.toHexString(
            infoById.get().getInternalTransactions(0).getCallerAddress().toByteArray()),
        ByteArray.toHexString(
            infoById.get().getInternalTransactions(0).getTransferToAddress().toByteArray()));

    Assert.assertEquals(
        ByteArray.toHexString(contractAddress2),
        ByteArray.toHexString(
    ...
    for (int i = 0; i < transactionsCount2; i++) {
      ...
      Assert.assertEquals(
          "call",
          ByteArray.toStr(infoById2.get().getInternalTransactions(i).getNote().toByteArray()));
    }
    Assert.assertEquals(
    ...
    Assert.assertEquals(
        ByteArray.toHexString(contractAddress1),
        ByteArray.toHexString(
    ...
    for (int i = 0; i < transactionsCount3; i++) {
      ...
      Assert.assertEquals(
          "call",
          ByteArray.toStr(infoById3.get().getInternalTransactions(i).getNote().toByteArray()));
    }
    Assert.assertEquals(
        ByteArray.toHexString(
            infoById3.get().getInternalTransactions(0).getCallerAddress().toByteArray()),
        ByteArray.toHexString(
  ...
  @Test(
      enabled = true,
      description = "Three-level nesting.Type " + "is create call->call->create call->suicide")
  ...
  public void testInternalTransaction016() {
    ...
    Assert.assertEquals(
        "suicide",
        ByteArray.toStr(infoById.get().getInternalTransactions(68).getNote().toByteArray()));
    Assert.assertEquals(
        "call",
        ByteArray.toStr(infoById.get().getInternalTransactions(67).getNote().toByteArray()));
    Assert.assertEquals(
        0, infoById.get().getInternalTransactions(67).getCallValueInfo(0).getCallValue());
    Assert.assertEquals(
        1, infoById.get().getInternalTransactions(68).getCallValueInfo(0).getCallValue());
    for (int i = 0; i < transactionsCount - 2; i++) {
      Assert.assertEquals(
          "create",
          ByteArray.toStr(infoById.get().getInternalTransactions(i).getNote().toByteArray()));
      Assert.assertEquals(
          1, infoById.get().getInternalTransactions(i).getCallValueInfo(0).getCallValue());
    }
    ...
    for (int i = 0; i < transactionsCount1; i++) {
      ...
      Assert.assertEquals(
          "create",
          ByteArray.toStr(infoById1.get().getInternalTransactions(i).getNote().toByteArray()));
    }
    dupInternalTrsansactionHash(infoById.get().getInternalTransactionsList());
  ...
  public void testInternalTransaction017() {
    ...
    Assert.assertEquals(
        "suicide",
        ByteArray.toStr(infoById.get().getInternalTransactions(76).getNote().toByteArray()));
    Assert.assertEquals(
        1000000 - 76,
  ...
  public void shutdown() throws InterruptedException {
    PublicMethed.freedResource(
        internalTxsAddress, testKeyForinternalTxsAddress, testNetAccountAddress, blockingStubFull);
    if (channelFull != null) {
  ...
  public void dupInternalTrsansactionHash(
    ...
    internalTransactionList.forEach(
        internalTransaction ->
            hashList.add(Hex.toHexString(internalTransaction.getHash().toByteArray())));
    List<String> dupHash =
        hashList.stream()
