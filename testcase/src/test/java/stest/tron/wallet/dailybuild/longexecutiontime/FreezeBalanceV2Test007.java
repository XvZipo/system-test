...
public class FreezeBalanceV2Test007 {
  ...
  public void test02FreeNetCanNotMakeNetWindowSizeChange() throws Exception {
    AccountResourceMessage accountResourceMessage =
        PublicMethed.getAccountResource(frozenBandwidthAddress, blockingStubFull);
    Long currentNetPrice =
        (1000000L * accountResourceMessage.getTotalNetWeight())
            / accountResourceMessage.getTotalNetLimit();
    currentNetPrice = Math.max(1L, currentNetPrice);

    Long needUnfreezeBalance =
        PublicMethed.getFrozenV2Amount(frozenBandwidthAddress, 0, blockingStubFull)
            - 275 * currentNetPrice
            - currentNetPrice * 100
    ...
    Assert.assertTrue(
        PublicMethed.unFreezeBalanceV2(
            frozenBandwidthAddress, frozenBandwidthKey, needUnfreezeBalance, 0, blockingStubFull));
    PublicMethed.waitProduceNextBlock(blockingStubFull);
