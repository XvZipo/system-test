...
public class WalletTestAccount014 {
  ...
  public void fullAndSoliMerged1ForFreeNetUsage() {
    ...
    Assert.assertTrue(
        PublicMethed.sendcoin(
            account014Address, 1000000000L, fromAddress, testKey002, blockingStubFull));

    // Test freeNetUsage in fullnode and soliditynode.
    Assert.assertTrue(
        PublicMethed.sendcoin(
            account014SecondAddress, 5000000L, account014Address, account014Key, blockingStubFull));
    Assert.assertTrue(
        PublicMethed.sendcoin(
            account014SecondAddress, 5000000L, account014Address, account014Key, blockingStubFull));
    ...
    Assert.assertTrue(
        freeNetUsageInSoliInFull > 0 && freeNetUsageInSolidity > 0 && freeNetUsageInFullnode > 0);
    Assert.assertTrue(
    ...
    Assert.assertTrue(
        freeNetUsageInFullnode <= freeNetUsageInSolidity + 5
            && freeNetUsageInFullnode >= freeNetUsageInSolidity - 5);
    Assert.assertTrue(
        createTimeInFullnode == createTimeInSolidity
            && createTimeInFullnode == createTimeInSoliInFull);
    Assert.assertTrue(createTimeInSoliInFull != 0);
    Assert.assertTrue(
        lastOperationTimeInFullnode == lastOperationTimeInSolidity
            && lastOperationTimeInFullnode == lastOperationTimeInSoliInFull);
    Assert.assertTrue(lastOperationTimeInSoliInFull != 0);
    Assert.assertTrue(
        lastCustomeFreeTimeInFullnode == lastCustomeFreeTimeInSolidity
  ...
  public void fullAndSoliMerged2ForNetUsage() {
    ...
    Assert.assertTrue(
        PublicMethed.freezeBalance(
            account014Address, 1000000L, 3, account014Key, blockingStubFull));
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    Assert.assertTrue(
        PublicMethed.sendcoin(
            account014SecondAddress, 1000000L, account014Address, account014Key, blockingStubFull));
    Assert.assertTrue(
        PublicMethed.freezeBalanceGetEnergy(
            account014Address, 1000000, 3, 1, account014Key, blockingStubFull));
    Assert.assertTrue(
        PublicMethed.freezeBalanceForReceiver(
            ...
            3,
            0,
            ByteString.copyFrom(account014SecondAddress),
            account014Key,
            blockingStubFull));
    Assert.assertTrue(
        PublicMethed.freezeBalanceForReceiver(
    ...
    Assert.assertTrue(
        PublicMethed.freezeBalanceForReceiver(
            account014SecondAddress,
            1000000,
            3,
            0,
            ByteString.copyFrom(account014Address),
            account014SecondKey,
            blockingStubFull));
    Assert.assertTrue(
        PublicMethed.freezeBalanceForReceiver(
    ...
    final long acquiredForBandwidthInFullnode =
        account014.getAcquiredDelegatedFrozenBalanceForBandwidth();
    final long delegatedBandwidthInFullnode = account014.getDelegatedFrozenBalanceForBandwidth();
    final long acquiredForEnergyInFullnode =
        account014.getAccountResource().getAcquiredDelegatedFrozenBalanceForEnergy();
    final long delegatedForEnergyInFullnode =
        account014.getAccountResource().getDelegatedFrozenBalanceForEnergy();
    ...
    final long acquiredForBandwidthInSoliInFull =
        account014.getAcquiredDelegatedFrozenBalanceForBandwidth();
    final long delegatedBandwidthInSoliInFull = account014.getDelegatedFrozenBalanceForBandwidth();
    final long acquiredForEnergyInSoliInFull =
        account014.getAccountResource().getAcquiredDelegatedFrozenBalanceForEnergy();
    final long delegatedForEnergyInSoliInFull =
        account014.getAccountResource().getDelegatedFrozenBalanceForEnergy();
    ...
    final long acquiredForBandwidthInSolidity =
        account014.getAcquiredDelegatedFrozenBalanceForBandwidth();
    final long delegatedBandwidthInSolidity = account014.getDelegatedFrozenBalanceForBandwidth();
    final long acquiredForEnergyInSolidity =
        account014.getAccountResource().getAcquiredDelegatedFrozenBalanceForEnergy();
    final long delegatedForEnergyInSolidity =
        account014.getAccountResource().getDelegatedFrozenBalanceForEnergy();

    logger.info("delegatedForEnergyInSolidity " + delegatedForEnergyInSolidity);
    Assert.assertTrue(netUsageInSoliInFull > 0 && netUsageInSolidity > 0 && netUsageInFullnode > 0);
    Assert.assertTrue(
    ...
    Assert.assertTrue(
        netUsageInFullnode <= netUsageInSolidity + 5
            && netUsageInFullnode >= netUsageInSolidity - 5);
    Assert.assertTrue(
        acquiredForBandwidthInFullnode == acquiredForBandwidthInSoliInFull
            && acquiredForBandwidthInFullnode == acquiredForBandwidthInSolidity);
    Assert.assertTrue(
        delegatedBandwidthInFullnode == delegatedBandwidthInSoliInFull
            && delegatedBandwidthInFullnode == delegatedBandwidthInSolidity);
    Assert.assertTrue(
        acquiredForEnergyInFullnode == acquiredForEnergyInSoliInFull
            && acquiredForEnergyInFullnode == acquiredForEnergyInSolidity);
    Assert.assertTrue(
        delegatedForEnergyInFullnode == delegatedForEnergyInSoliInFull
            && delegatedForEnergyInFullnode == delegatedForEnergyInSolidity);
    Assert.assertTrue(
        acquiredForBandwidthInSoliInFull == 1000000
            && delegatedBandwidthInSoliInFull == 1000000
            && acquiredForEnergyInSoliInFull == 1000000
            && delegatedForEnergyInSoliInFull == 1000000);
    logger.info("lastCustomeTimeInSoliInFull " + lastCustomeTimeInSoliInFull);
    Assert.assertTrue(
        lastCustomeTimeInFullnode == lastCustomeTimeInSolidity
  ...
  public void shutdown() throws InterruptedException {
    ...
    PublicMethed.freedResource(
        account014SecondAddress, account014SecondKey, fromAddress, blockingStubFull);
    if (channelFull != null) {
