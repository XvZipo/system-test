...
public class WalletTestBlock003 {
  ...
  public void beforeClass() {
    ...
    blockHash =
        PublicMethed.getBlock(blockNum + 1, blockingStubFull)
            .getBlockHeader()
            .getRawData()
            .getParentHash();
  }
  ...
  public void test01GetBlock() {
    ...
    while (!getBlockEqualGetNowBlock && retryTimes-- >= 0) {
      ...
      BlockExtention currentBlockFromGetNowBlock =
          blockingStubFull.getNowBlock2(GrpcAPI.EmptyMessage.newBuilder().build());
      logger.info(
          "currentBlockFromGetBlock: " + currentBlockFromGetBlock.getBlockHeader().toString());
      logger.info(
          "currentBlockFromGetNowBlock: "
              + currentBlockFromGetNowBlock.getBlockHeader().toString());

      if (currentBlockFromGetBlock
          .getBlockHeader()
          .equals(currentBlockFromGetNowBlock.getBlockHeader())) {
  ...
  public void test02GetBlockFromSolidity() {
    ...
    while (!getBlockEqualGetNowBlock && retryTimes-- >= 0) {
      ...
      logger.info(
          "test02GetBlockFromSolidity: currentBlockFromGetBlock"
              + currentBlockFromGetBlock.toString());
      logger.info(
          "test02GetBlockFromSolidity: currentBlockFromGetNowBlock"
              + currentBlockFromGetNowBlock.getBlockHeader().toString());
      if (currentBlockFromGetBlock
          .getBlockHeader()
          .equals(currentBlockFromGetNowBlock.getBlockHeader())) {
