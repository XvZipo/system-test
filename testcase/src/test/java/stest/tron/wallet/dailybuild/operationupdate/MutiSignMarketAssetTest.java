...
public class MutiSignMarketAssetTest {
  ...
  public void beforeClass() {
    ...
    Assert.assertTrue(
        PublicMethed.sendcoin(
            testAddress001, 20000_000000L, fromAddress, testKey002, blockingStubFull));
    ...
    Assert.assertTrue(
        PublicMethed.createAssetIssue(
            ...
            testKey001,
            blockingStubFull));
    Long balanceBefore = PublicMethed.queryAccount(testAddress001, blockingStubFull).getBalance();
    ...
    accountPermissionJson =
        "{\"owner_permission\":{\"type\":0,\"permission_name\":\"owner\",\"threshold\":2,\"keys\":["
            + "{\"address\":\""
            + PublicMethed.getAddressString(testKey001)
            + "\",\"weight\":1},"
            + "{\"address\":\""
    ...
    PublicMethedForMutiSign.accountPermissionUpdate(
        accountPermissionJson, testAddress001, testKey001, blockingStubFull, ownerKeyString);
  ...
  public void testMutiSignForMarketSellAsset001() {
    ...
    MarketOrderList marketOrder =
        PublicMethed.getMarketOrderByAccount(testAddress001, blockingStubFull).get();
  ...
  @Test(
      enabled = false,
      dependsOnMethods = "testMutiSignForMarketSellAsset001",
  ...
  public void testMutiSignForMarketOrderCancel001() {
    ...
    Assert.assertTrue(
        PublicMethedForMutiSign.marketCancelOrder(
            testAddress001, orderId.toByteArray(), 2, permissionKeyString, blockingStubFull));

    PublicMethed.waitProduceNextBlock(blockingStubFull);
    PublicMethed.waitProduceNextBlock(blockingStubFull);

    Assert.assertEquals(
        marketOrderCountBefore - 1,
        PublicMethed.getMarketOrderByAccount(testAddress001, blockingStubFull)
            .get()
            .getOrdersCount());
  }
  ...
  @Test(
      enabled = false,
      dependsOnMethods = "testMutiSignForMarketSellAsset002",
  ...
  public void testMutiSignForMarketOrderCancel002() {
    ...
    Assert.assertTrue(
        PublicMethedForMutiSign.marketCancelOrder(
            testAddress001, orderId.toByteArray(), 0, ownerKeyString, blockingStubFull));
