...
public class WalletTestTransactionMemo {
  ...
  public void beforeClass() {
    ...
    Assert.assertTrue(
        PublicMethed.sendcoin(
            memoAddress, sendAmount, foundationAddress, foundationKey, blockingStubFull));
  ...
  public void test01TransactionMemo() {
    ...
    String txid =
        PublicMethed.sendcoinWithMemoGetTransactionId(
            foundationAddress, sendAmount, memo, memoAddress, memoKey, blockingStubFull);
    ...
    String txidWithNoMemo =
        PublicMethed.sendcoinWithMemoGetTransactionId(
            foundationAddress, sendAmount, null, memoAddress, memoKey, blockingStubFull);
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    Long noMemoFreeNet =
        PublicMethed.getTransactionInfoById(txidWithNoMemo, blockingStubFull)
            .get()
            .getReceipt()
            .getNetUsage();
  ...
  public void test02TransactionMaxSize() {
    Assert.assertTrue(
        PublicMethed.sendcoinWithScript(
            memoAddress,
            10000000000L,
            foundationAddress,
            foundationKey,
  ...
  @Test(
      enabled = true,
      description = "transaction's max size is 1500 when create a new account (proposal 82)")
