...
public class NoAbi009 {
  ...
  public void testNoAbi001() {
    ...
    // getcontractinfo
    SmartContractDataWrapper contractInfo =
        PublicMethed.getContractInfo(contractAddress, blockingStubFull);
    ...
    List<String> retList =
        PublicMethed.getStrings(infoById.get().getLogList().get(0).getData().toByteArray());
    ...
    Assert.assertThat(
        transactionExtention.getResult().getCode().toString(), containsString("SUCCESS"));
    Assert.assertEquals(
        4, ByteArray.toInt(transactionExtention.getConstantResult(0).toByteArray()));
  }
  ...
  public void testNoAbi002() {
    ...
    // getcontractinfo
    SmartContractDataWrapper contractInfo =
        PublicMethed.getContractInfo(testNoABiContractAddress, blockingStubFull);
    System.out.println("contractInfo.toString():" + contractInfo.toString());
    Assert.assertTrue(contractInfo.getSmartContract().getAbi().toString().isEmpty());
    // Assert.assertTrue(contractInfo.getSmartContract().getName().equalsIgnoreCase(contractName));
    Assert.assertTrue(contractInfo.getRuntimecode().size() > 0);
    ...
    retList = PublicMethed.getStrings(infoById.get().getLogList().get(0).getData().toByteArray());
    logger.info("retList:" + retList.toString());
