...
public class KillContract01 {
  ...
  public void beforeClass() {
    ...
    Assert.assertTrue(
        PublicMethed.sendcoin(
            excAdd, 100000_000000L, testFoundationAddress, testFoundationKey, blockingStubFull));
    ...
    SmartContractOuterClass.SmartContract smartContract =
        PublicMethed.getContract(contractAddressD, blockingStubFull);
  ...
  @Test(
      enabled = true,
      description =
          "getPredictedAddress,sendcoin,transferAsset,and then make"
              + " create2,freezev2,delegateResource,delegateResource,unfreezeBalanceV2,cancelAllUnfreezeV2,vote,kill"
              + " into one tranaction")
  void kill01() {
    ...
    Assert.assertTrue(
        PublicMethed.transferAsset(
            create2AddBytes, assetAccountId.toByteArray(), 100L, excAdd, excKey, blockingStubFull));
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    ...
    Account delegateReceiverAccount =
        PublicMethed.queryAccount(delegateReceiverAdd, blockingStubFull);
    ...
    long targetAssetCount =
        PublicMethed.getAssetIssueValue(targetAdd, assetAccountId, blockingStubFull);
    Assert.assertEquals(100, targetAssetCount);
    Assert.assertEquals(1_000000L, delegateReceiverAccount.getBalance());
    AccountResourceMessage delegateReceiverResource =
        PublicMethed.getAccountResource(delegateReceiverAdd, blockingStubFull);
    Assert.assertEquals(0, delegateReceiverResource.getNetLimit());
    Assert.assertEquals(0, delegateReceiverResource.getEnergyLimit());
    AccountResourceMessage targetResource =
        PublicMethed.getAccountResource(targetAdd, blockingStubFull);
    Assert.assertNotEquals(0, targetResource.getNetLimit());
    Assert.assertNotEquals(0, targetResource.getEnergyLimit());
    long delegateReceiverAssetCount =
        PublicMethed.getAssetIssueValue(delegateReceiverAdd, assetAccountId, blockingStubFull);
    Assert.assertEquals(0, delegateReceiverAssetCount);
    long execAssetCount = PublicMethed.getAssetIssueValue(excAdd, assetAccountId, blockingStubFull);
  ...
  @Test(
      ...
      description =
          "getPredictedAddress,sendcoin,transferAsset,and then make"
              + " create2,freezev2,delegateResource,delegateResource,unfreezeBalanceV2,cancelAllUnfreezeV2,vote"
              + " into one transaction, and kill in another transaction. and can not static call"
              + " kill")
  void kill02() {
    String methedStr = "getPredictedAddress(bytes32)";
    String argsStr = "1122";
  ...
  @Test(
      enabled = true,
      description =
          "getPredictedAddress,sendcoin,transferAsset,and then "
  ...
  void kill03() {
    ...
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    SmartContractOuterClass.SmartContract smartContract =
        PublicMethed.getContract(contractC, blockingStubFull);
    Assert.assertFalse(smartContract.getAbi().toString().isEmpty());
    Assert.assertFalse(smartContract.getBytecode().toString().isEmpty());
    ...
    logger.info(
        "kill03 getPredictedAddress transactionExtention: " + transactionExtention.toString());
    String create2Add41 =
        "41"
            + ByteArray.toHexString(transactionExtention.getConstantResult(0).toByteArray())
                .substring(24);
    ...
    String args =
        argsStr + ",\"" + contractCBase58 + "\",\"" + contractBBase58 + "\",\"" + target03 + "\"";
    String txid1 =
        PublicMethed.triggerContract(
            contractAddressD,
            methedStr,
    ...
    Assert.assertEquals(0, targetAccount.getVotesList().size());
    long targetAssetCount =
        PublicMethed.getAssetIssueValue(targetAdd03, assetAccountId, blockingStubFull);
    Assert.assertEquals(100, targetAssetCount);
    AccountResourceMessage targetResource =
        PublicMethed.getAccountResource(targetAdd03, blockingStubFull);
    Assert.assertNotEquals(0, targetResource.getNetLimit());
    Assert.assertNotEquals(0, targetResource.getEnergyLimit());
  }
