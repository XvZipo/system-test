...
public class EnergyAdjustmentTest {
  ...
  public void beforeClass() {
    ...
    SmartContractOuterClass.SmartContract smartContract =
        PublicMethed.getContract(contractAddressC, blockingStubFull);
  ...
  @Test(
      enabled = true,
      description =
          "before committe no.81,memory extend does not cost fee, offset too big will result in cpu"
              + " time out. after committe no.81, offset too big will result in out of memory, and"
              + " offset le 3145696 will cost memory extend fee.")
  ...
  @Test(
      enabled = true,
      description =
          "before committe no.81,memory extend does not cost fee,"
  ...
  @Test(
      enabled = true,
      description = "sucide to active one account, active 1 account cost 25000 energy extra")
  ...
  void sucideToActiveAcount02() {
    ...
    String argsStr =
        "\""
            + Base58.encode58Check(contractAddressC)
            + "\",\""
  ...
  @Test(
      enabled = true,
      description =
          "call sucide twice to active two different accounts,"
  ...
  @Test(
      enabled = true,
      description = "sucide: receiver account only the last char is different with the contract")
  void sucideToActiveAcount04() {
    ...
    long balance =
        PublicMethed.queryAccount(ByteArray.fromHexString(sub), blockingStubFull).getBalance();
    Assert.assertEquals(trxValue, balance);
  }
  ...
  byte[] create2NewFreezeContract() {
    ...
    PublicMethed.sendcoin(
        create2AddBytes, trxValue, testFoundationAddress, testFoundationKey, blockingStubFull);
    PublicMethed.waitProduceNextBlock(blockingStubFull);
