...
public class NewFeatureForSolc0822 {
  ...
  public void beforeClass() {
    ...
    SmartContractOuterClass.SmartContract smartContract =
        PublicMethed.getContract(contractD, blockingStubFull);
  ...
  public void test001FileLevelEvent() {
    ...
    Assert.assertEquals(
        ByteArray.toHexString("abc".getBytes()),
        ByteArray.toHexString(transactionExtention.getLogs(0).getData().substring(64).toByteArray())
            .substring(0, 6));
    Assert.assertEquals(
        8,
    ...
    Assert.assertEquals(
        6, ByteArray.toInt(transactionExtention.getLogs(2).getData().toByteArray()));
  }

  @Test(
      enabled = true,
      description =
          "Fix internal error when requesting userdoc or devdoc for a contract "
  ...
  public void test002ImmutableInitInCondition() {
    ...
    Assert.assertEquals(
        ByteArray.toHexString(contractExcAddress).substring(2),
        ByteArray.toHexString(
            transactionExtention.getLogs(0).getTopics(1).substring(12).toByteArray()));
    Assert.assertEquals(
        8, ByteArray.toInt(transactionExtention.getLogs(0).getData().toByteArray()));
  }

  @Test(
      enabled = true,
      description =
          "Remove redundant overflow checks of certain ``for`` loops when the counter variable"
              + " cannot overflow.")
