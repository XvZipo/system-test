...
public class Opcode {
  ...
  public void test02Extcodecopy() {
    ...
    Assert.assertTrue(
        ByteArray.toHexString(infoById.get().getContractResult(0).toByteArray()).length() > 0);
  }

  @Test(
      enabled = true,
      description =
          "test opcode coinbase," + "block.coinbase is sr address which produced the block")
  public void test03Coinbase() {
    ...
    Assert.assertTrue(
        trueRes.startsWith("00000000000000000000000" + "0bafb56091591790e00aa05eaddcc7dc1474b5d4b")
            || trueRes.startsWith(
                "0000000000000000000000000be88a918d74d0dfd71dc84bd4abf036d0562991")
            || trueRes.startsWith(
                "0000000000000000000000003ed7d77d2eb807375a34e1a0043c5ba7e8926265"));
  }

  @Test(
      enabled = false,
      description =
          "test opcode difficulty,block.difficulty is always 0," + " Deprecate since 0.8.18")
  ...
  @Test(
      enabled = true,
      description = "test opcode pc,return current position, " + "ppppc() can refer to opCode.sol")
  public void test06Pc() {
    String code =
        "608060405260838060116000396000f3fe608060405234"
    ...
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    SmartContractOuterClass.SmartContract smartContract =
        PublicMethed.getContract(temContract, blockingStubFull);
    Assert.assertNotNull(smartContract.getAbi());
  ...
  @Test(
      enabled = true,
      description =
          "test opcode msize,return size of memory, "
              + "msize cannot be used if optimize is open, mmmsize() can refer to opCode.sol")
  public void test07Msize() {
    String code =
        "608060405260b5806100126000396000f3fe6080604052348015600f5760"
  ...
  @Test(
      enabled = true,
      description =
          "test opcode push13-30 but exclude push20 and push29," + "solidity cannot use optimize")
  ...
  @Test(
      enabled = true,
      description =
          "test opcode callcode,difference with delegatecall "
  ...
  public void test09Callcode() {
    String code =
        "60806040526103b4806100136000396000f3006080604052600436106100565763"
    ...
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    SmartContractOuterClass.SmartContract smartContract =
        PublicMethed.getContract(contractA, blockingStubFull);
    Assert.assertNotNull(smartContract.getAbi());

    code =
        "608060405260d2806100126000396000f300608060405260043610603e5763ffffffff7c0"
    ...
    txid =
        PublicMethed.triggerContract(
            contractA,
            "testIndelegateCall(address,uint256)",
            "\"" + base58C + "\",1",
            false,
            2,
            maxFeeLimit,
    ...
    txid =
        PublicMethed.triggerContract(
            contractA,
            "testInCallcode(address,uint256)",
            "\"" + base58C + "\",1",
            false,
            3,
            maxFeeLimit,
