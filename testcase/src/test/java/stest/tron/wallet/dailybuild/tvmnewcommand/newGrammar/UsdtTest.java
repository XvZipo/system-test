...
public class UsdtTest {
  ...
  @Test(
      enabled = true,
      description =
          "Test 4.7.1.1 hot fix of trigger "
              + "has no balance and no energyLimit when transfer USDT")
  public void test01() {

    Assert.assertTrue(
        PublicMethed.sendcoin(
            callerddress, 10000000L, testNetAccountAddress, testNetAccountKey, blockingStubFull));
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    Assert.assertTrue(
        PublicMethed.sendcoin(
            testNetAccountAddress, 10000000L, callerddress, callerKey, blockingStubFull));
    ...
    long origin_window_size_before =
        PublicMethed.queryAccount(dev001Address, blockingStubFull)
            .getAccountResource()
            .getEnergyWindowSize();
    argsStr = "\"" + dev58 + "\",1";
    txid =
  ...
  @Test(
      enabled = true,
      description =
          "Test 4.7.1.1 hot fix of trigger has "
  ...
  public void test03() {
    ...
    PublicMethed.delegateResourceV2(
        testNetAccountAddress, 300000000L, 0, callerddress, testNetAccountKey, blockingStubFull);
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    Assert.assertTrue(
        PublicMethed.sendcoin(
            testNetAccountAddress, balance, callerddress, callerKey, blockingStubFull));
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    String methedStr = "transfer(address,uint256)";
    String argsStr = "\"" + dev58 + "\",1";
    long origin_window_size_before =
  ...
  public void test04() {
    ...
    long balance = PublicMethed.queryAccount(callerddress, blockingStubFull).getBalance();
    PublicMethed.delegateResourceV2(
        testNetAccountAddress, 600000000L, 1, callerddress, testNetAccountKey, blockingStubFull);
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    if (balance != 0) {
      Assert.assertTrue(
          PublicMethed.sendcoin(
  ...
  public void shutdown() throws InterruptedException {
    ...
    PublicMethed.unDelegateResourceV2(
        testNetAccountAddress, 1000000000L, 1, dev001Address, testNetAccountKey, blockingStubFull);
    PublicMethed.unDelegateResourceV2(
        testNetAccountAddress, 300000000L, 0, callerddress, testNetAccountKey, blockingStubFull);
