...
public class FreezeSuicideTest001 {
  ...
  public void beforeClass() throws Exception {
    ...
    Assert.assertTrue(
        PublicMethed.freezeBalanceGetEnergy(
            testAddress002, 100_0000_000000L, 0, 0, testKey002, blockingStubFull));
  }
  ...
  public void FreezeSuicideTest002() {
    ...
    Account contractAccount_before = PublicMethed.queryAccount(contractAddress, blockingStubFull);
    AccountResourceMessage freezeAccount_before =
    ...
    AccountResourceMessage freezeAccount_after =
        PublicMethed.getAccountResource(testAddress002, blockingStubFull);
    Account contractAccount_after = PublicMethed.queryAccount(contractAddress, blockingStubFull);

    Assert.assertEquals(
        freezeCount.longValue(), contractAccount_after.getFrozen(0).getFrozenBalance());
    Assert.assertEquals(
        contractAccount_before.getBalance() - freezeCount, contractAccount_after.getBalance());
    ...
    freezeAccount_after = PublicMethed.getAccountResource(testAddress002, blockingStubFull);
    Assert.assertEquals(freezeAccount_before.getNetLimit(), freezeAccount_after.getNetLimit());
  ...
  @Test(
      enabled = true,
      description = "suicide、freeze、unfreeze、getExpireTime " + "with suicided create2 address")
  public void FreezeSuicideTest003() {
    ...
    Assert.assertEquals(
        contract_after.getAccountResource().getDelegatedFrozenBalanceForEnergy(),
