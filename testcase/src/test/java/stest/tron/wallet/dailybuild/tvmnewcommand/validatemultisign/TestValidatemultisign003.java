...
public class TestValidatemultisign003 {
  ...
  public void test001DeployContract() {
    ...
    Assert.assertTrue(
        PublicMethed.freezeBalanceForReceiver(
            fromAddress,
            100_000_000L + PublicMethed.randomFreezeAmount.addAndGet(1),
            0,
            0,
    ...
    Assert.assertTrue(
        PublicMethed.sendcoin(
            ownerAddress, needCoin + 2048000000L, fromAddress, testKey002, blockingStubFull));
    PublicMethed.waitProduceNextBlock(blockingStubFull);
  ...
  @Test(
      enabled = true,
      description = "Trigger validatemultisign precompiled contract, " + "with wrong hash bytes")
  public void test002validatemultisign() {
    ...
    Transaction transaction =
        PublicMethedForMutiSign.sendcoinGetTransaction(
    ...
    List<Object> parameters =
        Arrays.asList(
            WalletClient.encode58Check(ownerAddress), 0, "0x" + Hex.toHexString(hash), signatures);
    String argsStr = PublicMethed.parametersString(parameters);

    byte[] inputBytesArray =
        Hex.decode(
            AbiUtil.parseMethod(
    ...
    String TriggerTxid =
        PublicMethed.triggerContract(
            contractAddress,
            methodStr,
            AbiUtil.parseParameters(methodStr, Arrays.asList(input)),
            true,
            0,
            100000000,
            dev001Address,
  ...
  @Test(
      enabled = true,
      description = "Trigger validatemultisign precompiled contract, " + "with correct hash bytes")
  public void test003validatemultisign() {
    ...
    String input =
        AbiUtil.parseParameters("validatemultisign(address,uint256,bytes32,bytes[])", argsStr);
