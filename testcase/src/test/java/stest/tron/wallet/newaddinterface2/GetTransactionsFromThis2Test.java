...
@Slf4j
public class GetTransactionsFromThis2Test extends AbstractGrpcFullSolidityExtensionTest {
  ...
  public void testgetTransactionsFromThis2() {
    // Create a transfer.
    Assert.assertTrue(
        PublicMethed.sendcoin(toAddress, 1000000, fromAddress, testKey002, blockingStubFull));
    ...
    if (gettransactionsfromthis2.get().getTransactionCount() == 0) {
      Assert.assertTrue(
          PublicMethed.sendcoin(toAddress, 1000000L, fromAddress, testKey002, blockingStubFull));
    }
