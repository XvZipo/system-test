...
public class JsonRpcTest extends JsonRpcBase {
  ...
  public void checkTrxBalance(ByteString address) throws Exception {
    ...
    Assert.assertEquals(
        ...
        PublicMethed.queryAccount(address.toByteArray(), blockingStubFull).getBalance());
  }
  ...
  public void test03StateTreeWithEthCall() throws Exception {
    ...
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    Assert.assertTrue(
        PublicMethed.getTransactionInfoById(trc20Txid, blockingStubFull).get().getLogCount() == 1);
